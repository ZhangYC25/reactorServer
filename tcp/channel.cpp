...
void Channel::set_read_callback(std::function<void()> const& _cb){
    read_callback_ = std::move(_cb);
}

void Channel::set_write_callback(std::function<void()> const& _cb) {
    write_callback_ = std::move(_cb);
}
